...
const categoryColors = {
    ...
    'Technical Knowledge': 'bg-purple-50 text-purple-700 border-purple-200',
...
const stageConfig = {
    'Excellent': { color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200', icon: '🌟' },
    'Good': { color: 'text-blue-600', bg: 'bg-blue-50 border-blue-200', icon: '👍' },
    'Developing': { color: 'text-amber-600', bg: 'bg-amber-50 border-amber-200', icon: '📈' },
    'Needs Improvement': { color: 'text-red-500', bg: 'bg-red-50 border-red-200', icon: '💪' }
}
...
export default function InterviewPage() {
    ...
    useEffect(() => {
        ...
        fetch('/api/interview/generate', {
            ...
            body: JSON.stringify({ resume_text: resumeText, role, count: 7 })
        })
            .then(r => r.json())
            .then(data => {
    ...
    const handleEvaluate = async () => {
        ...
        try {
            const res = await fetch('/api/interview/evaluate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ question: q.question, answer, role, category: q.category })
            })
        ...
        return (
            <div className="max-w-xl mx-auto space-y-4">
                ...
                <div className="card text-center space-y-4">
                    <BriefcaseBusiness size={32} className="text-slate-300 mx-auto" />
                    <p className="text-slate-600">Please upload your resume first — the interview questions are generated from your actual resume content.</p>
                    <button onClick={() => navigate('/dashboard/resume')} className="btn-primary mx-auto">Upload Resume</button>
                </div>
            </div>
        )
    }
        ...
        return (
            <div className="max-w-2xl mx-auto card flex items-center gap-3 text-brand-600">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">This may take 10–20 seconds</p>
                </div>
            </div>
        )
    }
    ...
    if (error || questions.length === 0) {
        return (
            <div className="max-w-xl mx-auto space-y-4">
                <h1 className="section-heading">Resume Interview Simulator</h1>
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600">{error || 'Could not generate questions'}</div>
            </div>
        )
    }
        ...
        return (
            <div className="max-w-lg mx-auto space-y-5">
                <div className="card text-center space-y-4">
                    ...
                    <h2 className="text-2xl font-extrabold text-slate-800">Interview Complete!</h2>
                    ...
                    <p className="text-sm text-slate-500">{evalValues.length} of {questions.length} answers evaluated</p>
                    ...
                    <div className="space-y-3 text-left">
                        {questions.map((q, i) => {
                            ...
                            return (
                                <div key={i} className={`p-3 rounded-xl border text-sm ${c ? c.bg : 'bg-surface-50 border-surface-200'}`}>
                                    <p className={`font-semibold ${c ? c.color : 'text-slate-500'} flex items-center gap-1.5`}>
                                        <span>{c ? c.icon : '⬜'}</span>
                                        Q{i + 1}: {q.question.substring(0, 80)}...
                                    </p>
                                    {ev && <p className="text-slate-600 mt-1 text-xs leading-relaxed">{ev.feedback}</p>}
                                    {!ev && <p className="text-slate-400 mt-1 text-xs italic">Not answered</p>}
                                </div>
                            )
                        })}
                    </div>
    ...
    const q = questions[current]
    const catClass = categoryColors[q.category] || 'bg-slate-50 text-slate-600 border-slate-200'
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-5">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="section-heading flex items-center gap-2">
                        <Mic size={20} className="text-brand-600" /> Resume Interview
                    </h1>
            ...
            <div className="flex gap-1.5">
                {questions.map((_, i) => (
                    <div key={i} className={`flex-1 h-1.5 rounded-full transition-all ${evaluations[i] ? 'bg-emerald-500' :
            ...
            <div className="card space-y-4">
                ...
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Your Answer</label>
                    <textarea
                        className="w-full min-h-[140px] p-3 text-sm text-slate-700 bg-surface-50 border border-surface-200 rounded-xl resize-y focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent transition-all"
            ...
            {ev && evCfg && (
                <div className={`card border space-y-4 animate-fade-in ${evCfg.bg}`}>
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed">{ev.feedback}</p>
                    ...
                    {showHint && q.hint && (
                        <div className="p-3 bg-white rounded-xl border border-slate-200 flex items-start gap-2">
                            ...
                            <p className="text-xs text-slate-600"><strong>Interviewer's tip:</strong> {q.hint}</p>
                        </div>
                    )}
                </div>
            )}
